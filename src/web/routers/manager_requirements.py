...
async def get_current_manager_user(user: dict = Depends(get_current_user_from_cookie)) -> dict:
    print(f"get_current_manager_user received user: {user}")
...
async def create_requirement(
    ...
    """採用要件を作成"""
    print("create_requirement function called.")
    try:
        ...
        print(f"Parsed structured_data: {parsed_structured_data}")
        ...
        print(f"Attempting to insert new requirement: {new_requirement}")
        ...
        return RedirectResponse(
            url=f"/manager/requirements/new?error=validation_failed&message={e}",
            status_code=303
        )
    except Exception as e:
        print(f"Error creating requirement: {e}")
        return RedirectResponse(
            url="/manager/requirements/new?error=creation_failed",
            status_code=303
        )
...
async def view_requirement(
    ...
    try:
        ...
        print(f"Error in view_requirement: {e}")
        raise HTTPException(status_code=404, detail="Requirement not found")
