...
from ..middleware.extension_auth import get_extension_user
...
async def get_all_requirements(
    ...
    try:
        ...
        print(f"Requirements query result: {len(result.data)} items")
        if result.data:
            print(f"First requirement sample: {result.data[0]}")
        ...
        for req in result.data:
            ...
            # デバッグ: 元のデータを確認
            print(f"Original requirement keys: {req.keys()}")
            print(f"client_id in original: {'client_id' in req}")
            ...
            if 'client_id' not in requirement:
                if 'client' in requirement and requirement['client'] and 'id' in requirement['client']:
                    ...
                    print(f"Warning: No client_id found for requirement {requirement.get('id')}")
...
async def start_scraping_session(
    ...
    """スクレイピングセッションを開始"""
    try:
        print(f"Starting scraping session - client_id: {request.client_id}, requirement_id: {request.requirement_id}")
        ...
        print(f"Session data to insert: {session_data}")
        
        result = supabase.table('scraping_sessions').insert(session_data).execute()
        
        if not result.data:
            print(f"Failed to create session. Result: {result}")
            raise HTTPException(
                status_code=500,
