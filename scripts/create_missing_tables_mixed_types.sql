-- Supabaseで不足しているテーブルを作成するSQL（混在型対応版）
-- 実行日: 2025-07-10
-- 注意: 各テーブルの実際の型に合わせて外部キーを設定

-- まず既存テーブルの型を確認
-- 以下のクエリを実行して型を確認してください：
/*
SELECT 
    table_name,
    column_name,
...
AND column_name = 'id'
AND table_name IN ('profiles', 'clients', 'job_requirements', 'candidates')
ORDER BY table_name;
*/

-- エラーメッセージから推測される型：
-- clients.id = UUID型
-- job_requirements.id = TEXT型（前回のエラーより）
-- candidates.id = TEXT型（前回のエラーより）
-- profiles.id = おそらくUUID型（auth.uid()がUUIDを返すため）

-- 1. client_settings（クライアント別設定）テーブル
CREATE TABLE IF NOT EXISTS client_settings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    client_id UUID NOT NULL REFERENCES clients(id) ON DELETE CASCADE,  -- clientsテーブルのidはUUID型
...
CREATE TABLE IF NOT EXISTS ai_evaluations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    candidate_id TEXT NOT NULL REFERENCES candidates(id) ON DELETE CASCADE,  -- candidatesテーブルのidはTEXT型
    requirement_id TEXT NOT NULL REFERENCES job_requirements(id) ON DELETE CASCADE,  -- job_requirementsテーブルのidはTEXT型
    search_id TEXT,
...
CREATE TABLE IF NOT EXISTS searches (
    ...
    error_message TEXT,
    search_params JSONB,
    created_by UUID REFERENCES profiles(id),  -- profilesテーブルのidはおそらくUUID型
    created_at TIMESTAMPTZ DEFAULT NOW()
);
