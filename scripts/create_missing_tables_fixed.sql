-- Supabaseで不足しているテーブルを作成するSQL（型修正版）
-- 実行日: 2025-07-10
-- 注意: 既存テーブルのID型に合わせて修正済み

-- 1. client_settings（クライアント別設定）テーブル
CREATE TABLE IF NOT EXISTS client_settings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    client_id TEXT NOT NULL REFERENCES clients(id) ON DELETE CASCADE,  -- clientsテーブルのidはTEXT型
...
CREATE TABLE IF NOT EXISTS searches (
    ...
    error_message TEXT,
    search_params JSONB,
    created_by TEXT REFERENCES profiles(id),  -- profilesテーブルのidはTEXT型
    created_at TIMESTAMPTZ DEFAULT NOW()
);
