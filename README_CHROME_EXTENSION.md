...
- **BigQuery:**
  - **役割:** 大規模データの蓄積と分析。スクレイピングで収集された生データ、AI判定結果、システムログなど、**大量の履歴データや分析対象となるデータ**を保存する「公式の記録保管庫」です。データの分析や、過去の判定傾向の確認などに利用します。
...
このセクションでは、Chrome拡張機能版の開発タスクを詳細に分解したWBS（Work Breakdown Structure）を提示します。
...
gcloud run deploy rpo-webapp \
    ...
    --set-env-vars CORS_ORIGINS="chrome-extension://[EXTENSION_ID]"
```

### Cloud Functions（AI判定）のデプロイ

```bash
# AI判定処理のデプロイ
gcloud functions deploy rpo-ai-evaluator \
    --runtime python39 \
    --trigger-http \
    --entry-point evaluate_candidates \
    --set-env-vars OPENAI_API_KEY=[YOUR_KEY]

# バッチ処理のデプロイ
gcloud functions deploy rpo-batch-processor \
    --runtime python39 \
    --trigger-http \
    --entry-point process_job_completion \
    --timeout 540s
```
